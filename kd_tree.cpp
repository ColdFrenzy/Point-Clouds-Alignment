...
/*double BaseTreeNode::findNeighbor(Eigen::VectorXd& answer, 
...
//! ctor
LeafNode::LeafNode(int dimension_):BaseTreeNode(dimension_){}

//! const accessor to the point vector
const VectorXdVector& LeafNode::points() const {return _points;}

//! accessor to the point vector
VectorXdVector& LeafNode::points()  {return _points;} 
...
//! @param maximum distance allowed for a point
//! @returns the distance of the closest point. -1 if no point found within range
double LeafNode::findNeighbor(Eigen::VectorXd& answer,
                            const Eigen::VectorXd& query, 
...
inline bool MiddleNode::side(const Eigen::VectorXd& query_point) const {
...
//! @param maximum distance allowed for a point
//! @returns the distance of the closest point. -1 if no point found within range
double MiddleNode::findNeighbor(Eigen::VectorXd& answer,
                    const Eigen::VectorXd& query, 
                    const double max_distance) const{
